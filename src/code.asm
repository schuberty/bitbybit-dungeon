...
generateBmBg:	li   $t0, 0x1004	# Buffer para preencher fundo
	sll  $t0, $t0, 16	#
...
genBm1:	sw   $t6, ($t0)		#
	j    genBmNext1		#
genBm2:	sw   $t7, ($t0)		#
genBmNext1:	div  $t4, $t1		# Pula caso seja a area do painel
	beqz $t4, genBmNext2	#
	mfhi $t3		#
	beqz $t3, genBmIfPanel	#
	add  $t0, $t0, 4	# Incrementa o buffer
genBmNext2:	add  $t4, $t4, 1	# Incrementa para o proximo pixel
	blt  $t4, 1537, genBmLoop	# Checa se preencheu toda a area do mapa
	jr   $ra		#
genBmIfPanel:	add  $t0, $t0, 68	# Incrementa para a proxima linha por ser a area do painel
	j    genBmNext2		#
#################################################
# Gera o desenho inicial dos HP		#
generateLife:	li   $t0, 0x1004	# Carrega o buffer para desenhar a vida inicial
	sll  $t0, $t0, 16	#
	add  $t0, $t0, 464	#
	li   $t1, 0xFF0000	# Cores da vida
	li   $t2, 0xD3A4A4	# 
	li   $t3, 14		# Comeca a desenhar a vida
genManaLoop1:	sw   $t1, ($t0)		#
	add  $t0, $t0, 4	#
	sw   $t1, ($t0)		#
	add  $t0, $t0, 252	#
	add  $t3, $t3, -1	#
...
generateMana:	li   $t0, 0x1004	# Carrega o buffer para desenhar a mana inicial
	sll  $t0, $t0, 16	#
	add  $t0, $t0, 488	#
	li   $t1, 0x000FFF	# Cores da mana
	li   $t2, 0xAAAFFF	# 
	li   $t3, 14		# Comeca a desenhar a mana
genLifeLoop1:	sw   $t1, ($t0)		#
	add  $t0, $t0, 4	#
	sw   $t1, ($t0)		#
	add  $t0, $t0, 252	#
	add  $t3, $t3, -1	#
...
generateRooms:	li   $t0, 0		# Numero de salas da dungeon zerado
	li   $t1, 0x1004	# Buffer
	sll  $t1, $t1, 16	#
	...
	li   $a1, 7		#
genRmMinRooms:	syscall		# $t0 = (int)(2 < x < 7)
